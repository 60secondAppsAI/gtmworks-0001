        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalApprovals = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalApprovals">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Approval</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="ApprovalId" type="text" placeholder="Enter ApprovalId" v-model="approvalToAdd.approvalId"></base-input>
  <base-input label="ApprovalType" type="text" placeholder="Enter ApprovalType" v-model="approvalToAdd.approvalType"></base-input>
  <base-input label="Status" type="text" placeholder="Enter Status" v-model="approvalToAdd.status"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="approvals" :row-key="record => record.ApprovalId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <ApprovalPictureView :approvals="approvals" />
          </div>
...
import ApprovalPictureView from './ApprovalPictureView.vue';
...
export default {
    ...
    return {
      ...
      columns: [
		...
		dataIndex: 'approvalId',
          visible: true,
          scopedSlots: { customRender: 'approvalId' },
          sorter: true
          //sorter: (a, b) => a.approvalId - b.approvalId,
          //sorter: (a, b) => a.approvalId.localeCompare(b.approvalId),
        },
        {
          title: 'Approval Type',
		dataIndex: 'approvalType',
          ...
          sorter: true
          //sorter: (a, b) => a.approvalType - b.approvalType,
      ...
      sortBy: 'approvalId',           // Column to sort by
  ...
  methods: {
    ...
    async renderApprovalsTable() {
      ...
      let approvalsTableData = [];
      for (let i = 0; i < this.approvals.length; i++) {
        approvalsTableData.push({
          id: i,
          approvalId: this.approvals[i].approvalId,
          year: this.approvals[i].year,
          date: this.approvals[i].date,
          competitionId: this.approvals[i].competitionId,
          approvalId: this.approvals[i].approvalId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-approvals',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForApprovalsChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalApprovals = false;

      const currentDate = new Date().getTime();
      this.approvalToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.approvalToAdd);
      console.log(jsonData);
      
      const res = await ApprovalService.addApproval(jsonData);
