        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalRevenues = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalRevenues">
                <template slot="header">
                  <h5 class="modal-title" id="exampleModalLabel">Add Revenue</h5>
                </template>
                <div>
                  <form @submit.prevent>
  <base-input label="RevenueId" type="text" placeholder="Enter RevenueId" v-model="revenueToAdd.revenueId"></base-input>
  <base-input label="Year" type="text" placeholder="Enter Year" v-model="revenueToAdd.year"></base-input>
  <base-input label="Quarter" type="text" placeholder="Enter Quarter" v-model="revenueToAdd.quarter"></base-input>
  <base-input label="RevenueAmount" type="text" placeholder="Enter RevenueAmount" v-model="revenueToAdd.revenueAmount"></base-input>
                  </form>
                </div>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="revenues" :row-key="record => record.RevenueId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <RevenuePictureView :revenues="revenues" />
          </div>
...
import RevenuePictureView from './RevenuePictureView.vue';
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'revenueId',
          visible: true,
          scopedSlots: { customRender: 'revenueId' },
          sorter: true
          //sorter: (a, b) => a.revenueId - b.revenueId,
          //sorter: (a, b) => a.revenueId.localeCompare(b.revenueId),
        },
        {
          title: 'Year',
		dataIndex: 'year',
		...
		dataIndex: 'revenueAmount',
          visible: true,
          scopedSlots: { customRender: 'revenueAmount' },
          sorter: true
          //sorter: (a, b) => a.revenueAmount - b.revenueAmount,
          //sorter: (a, b) => a.revenueAmount.localeCompare(b.revenueAmount),
        },
      ],
      pagination: {
        current: 1,
        ...
        showTotal: (total) => `Total ${total} revenues`,
      },
      ...
      sortBy: 'revenueId',           // Column to sort by
  ...
  methods: {
    ...
    async renderRevenuesTable() {
      ...
      let revenuesTableData = [];
      for (let i = 0; i < this.revenues.length; i++) {
        revenuesTableData.push({
          id: i,
          revenueId: this.revenues[i].revenueId,
          year: this.revenues[i].year,
          date: this.revenues[i].date,
          competitionId: this.revenues[i].competitionId,
          revenueId: this.revenues[i].revenueId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-revenues',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForRevenuesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      this.modalRevenues = false;

      const currentDate = new Date().getTime();
      this.revenueToAdd.created = currentDate;

      const jsonData = JSON.stringify(this.revenueToAdd);
      console.log(jsonData);
      
      const res = await RevenueService.addRevenue(jsonData);
