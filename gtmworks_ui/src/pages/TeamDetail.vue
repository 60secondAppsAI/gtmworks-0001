					...
					<tr> 
						<td style="width: 100%;"></td>
				        	...
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Teams = true">Add</base-button>
								              <modal :show.sync="modal2Teams">
								                <template slot="header">
								                  <h5 class="modal-title" id="exampleModalLabel">Add Team</h5>
								                </template>
								                <div>
								                  <form @submit.prevent>
								  <base-input label="TeamId" type="text" placeholder="Enter TeamId" v-model="teamToAdd.teamId"></base-input>
								  <base-input label="TeamName" type="text" placeholder="Enter TeamName" v-model="teamToAdd.teamName"></base-input>
												                  </form>
								                </div>
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Team Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="TeamId" type="text" placeholder="Enter TeamId" v-model="teamDetails.teamId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Team Name</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="TeamName" type="text" placeholder="Enter TeamName" v-model="teamDetails.teamName"></input>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async handleAddSubmitted() {
      this.modal2Teams = false;

      const jsonData = JSON.stringify(this.teamToAdd);
...
//        this.getAllTeams();
    ...
    async getTeamDetails() {
      try {
			...
			this.answers = this.teamDetails.answers;
			hasAnswers = 1;
